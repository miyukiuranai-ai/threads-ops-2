...
export const TOOLS = [
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        dayMix: { type: 'array', items: { type: 'string', enum: Object.keys(POST_TYPES) }, description: 'いまの要望の構成（変えるまで続く）。型の割合として使う。例: ["buzz_engagement","buzz_engagement","attract_intro"]＝バズ2：属人1。本数（postsPerDay）を別に指定すると、その本数に割合で割り振る。指定しなければ構成の数がそのまま本数になる。空配列で解除して大元のルールに戻す。1日だけ変えるなら set_day_directive の mix を使う' },
...
export async function runTool(name, input, ctx) {
  ...
  if (name === 'update_persona') {
    ...
    if (Array.isArray(input.dayMix)) {
      ...
      upd.dayMix = mix;
      // 本数を一緒に指定されたときはそちらを優先する。
      // 指定が無ければ構成の数をそのまま本数にする（これまでどおり）。
      // 本数と構成の数が違う日は、構成を「割合」として割り振る（属人3＋バズ2で3本なら属人2＋バズ1）。
