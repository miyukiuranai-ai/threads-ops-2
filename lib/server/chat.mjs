...
import { listAccounts, getPersonaForAccount, updatePersona, resolveAccount, updateAccount } from './accounts.mjs';
...
async function runTool(name, input, session) {
  ...
  switch (name) {
    ...
    case 'update_persona': {
      ...
      const pid = persona.id || acc.personaId || acc.id;
      await updatePersona(pid, patch);
      if (acc.personaId !== pid) await updateAccount(acc.id, { personaId: pid });
      return { persona: pid, updated: Object.keys(patch) };
    }
