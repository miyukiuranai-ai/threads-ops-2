...
export async function takeSnapshots(accounts, { now = new Date() } = {}) {
  ...
  const results = [];
  const since = new Date(now.getTime() - 4 * 3600000).toISOString();
  for (const acc of accounts) {
    if (!acc.accessToken) continue;
    const hist = await listDocs('history', { where: [['accountId', '==', acc.id], ['timestamp', '>=', since]] });
    for (const h of hist) {
...
export async function latestVerdict(accountId, { now = new Date() } = {}) {
  ...
  const hist = await listDocs('history', { where: [['accountId', '==', accountId], ['timestamp', '>=', since]] });
  const withV = hist.filter((h) => h.verdict3h && !h.deletedAt).sort((a, b) => (b.timestamp || '').localeCompare(a.timestamp || ''));
  return withV[0] || null;
}

/** 直近の 3 時間後判定の並び（新しい順） */
export async function recentVerdicts(accountId, { limit = 5 } = {}) {
  const hist = await listDocs('history', { where: [['accountId', '==', accountId]], limit: 300 });
  return hist.filter((h) => h.verdict3h && !h.deletedAt).sort((a, b) => (b.timestamp || '').localeCompare(a.timestamp || '')).slice(0, limit);
}
