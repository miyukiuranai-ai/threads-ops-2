...
/** その型がどの段か。0=超バズ特化 1=バズ特化 2=属人系。構成（dayMix）の割合はこの段で合わせる。 */
export function tierOf(type) {
  const o = MIX_ORDER[type] ?? 9;
  if (o === 0) return 0;
  if (o <= 2) return 1;
  return 2;
}

/** 日付から日替わりの番号を作る（同じ日なら何度呼んでも同じ）。 */
...
/**
 * 構成（dayMix）を「割合」として、その日の本数に割り振る。
 * 例: 属人系3・バズ特化2（＝6:4）の構成なら、3本の日は属人2＋バズ1、4本の日は属人2＋バズ2になる。
 * 本数が構成の数とちょうど同じ日は構成そのままになるので、これまでの名義の動きは変わらない。
 * 段ごとの本数は割合に近い整数に丸め（端数の大きい段から1本ずつ足す）、
 * 同じ段に型が複数あるときは日替わりで順ぐりに使う（毎日同じ型ばかりにしない）。
 */
export function spreadMix(mix, n, dayIndex = 0) {
  if (!mix.length || n <= 0) return [];
  const tiers = [];
  for (const t of mix) {
    const tier = tierOf(t);
    let e = tiers.find((x) => x.tier === tier);
    if (!e) { e = { tier, types: [] }; tiers.push(e); }
    e.types.push(t);
  }
  const quota = tiers.map((e) => (e.types.length * n) / mix.length);
  const counts = quota.map((q) => Math.floor(q));
  // 端数がちょうど半分で並んだとき（例: 5:5 の3本）は、どちらに1本足すかを日替わりで交代させる。
  // そうしないと毎日同じ側ばかりが1本多くなり、指定した割合から少しずつずれていく。
  const tieBreak = (i) => ((i + dayIndex) % quota.length + quota.length) % quota.length;
  const byFrac = quota
    .map((q, i) => ({ i, frac: q - Math.floor(q) }))
    .sort((a, b) => b.frac - a.frac || tieBreak(a.i) - tieBreak(b.i));
  let rest = n - counts.reduce((a, b) => a + b, 0);
  for (let k = 0; rest > 0; k += 1, rest -= 1) counts[byFrac[k % byFrac.length].i] += 1;

  const out = [];
  tiers.forEach((e, i) => {
    const start = ((dayIndex % e.types.length) + e.types.length) % e.types.length;
    for (let k = 0; k < counts[i]; k += 1) out.push(e.types[(start + k) % e.types.length]);
  });
  return orderMix(out);
}
...
export function planDay(persona, { slots = null, types = null, limit = null, floor = null, date = null } = {}) {
  if (slots) {
  ...
  const typesFromMix = (n) => shuffleTypes(spreadMix(mix, n, dayIndexOf(date)));
...
export async function generateForAccount(account, { types, slots, date, dry = false, instruction = '' } = {}) {
  ...
  const planned = planDay(dayMixRequest ? { ...persona, dayMix: dayMixRequest, postsPerDay: String(dayMixRequest.length) } : persona, { slots, types, limit, floor, date: date ?? tomorrowJst() });
