...
import { stockSummary, textStockSummary, pickStock, pickPlaceStock, markStockUsed, stockToMedia } from './stock.mjs';
...
function hintedGenres(text, genres) {
  const t = String(text ?? '');
  const out = [];
...
export async function listGeneratableAccounts(accountName = null) {
  const db = getDb();
...
export async function generateDaily({ date, accountName = null } = {}) {
  const db = getDb();
  const startedAt = new Date().toISOString();
  const accounts = await listGeneratableAccounts(accountName);
  const results = [];
