...
export async function listPosts({ accountId, accountIds, status, statuses, plannedDate, from, to, limit = 500, orderBy = ['scheduledAt', 'asc'] } = {}) {
  ...
  if (from) where.push(['scheduledAt', '>=', from]);
  if (to) where.push(['scheduledAt', '<=', to]);
  let list = await listDocs('posts', { where, orderBy: where.some((w) => w[0] === 'scheduledAt') || !where.length ? orderBy : null, limit });
  if (accountIds?.length) list = list.filter((p) => accountIds.includes(p.accountId));
  if (orderBy) {
    const [f, dir] = orderBy;
    list.sort((a, b) => ((a[f] || '') < (b[f] || '') ? -1 : 1) * (dir === 'desc' ? -1 : 1));
  }
  return list;
}
