...
export async function listHistory({ accountId, from, to, limit = 1000 } = {}) {
  // 名義があれば等値だけで引き、期間はメモリで絞る（複合インデックス不要）
  let list;
  if (accountId) {
    list = await listDocs('history', { where: [['accountId', '==', accountId]], limit: 3000 });
  } else {
    const where = [];
    if (from) where.push(['timestamp', '>=', from]);
    if (to) where.push(['timestamp', '<=', to]);
    list = await listDocs('history', { where, limit: 3000 });
  }
  if (from) list = list.filter((h) => (h.timestamp || '') >= from);
  if (to) list = list.filter((h) => (h.timestamp || '') <= to);
  return list.filter((h) => !h.deletedAt).sort((a, b) => (b.timestamp || '').localeCompare(a.timestamp || '')).slice(0, limit);
}
